...
def main(argv):
    if len(argv) < 2:
        print("usage: bot_balance_gate_v4b.py results.csv")
        return 2
    ...
    if not (35 <= red <= 70):
        fails.append("RED% outside 35-70")
    if blue < 25:
        fails.append("BLUE% < 25")
    if close < 45:
        fails.append("close<=6 < 45")
    if avg_gap > 8.5:
        fails.append("avg_gap > 8.5")
    if b12 > 35:
        fails.append("blowout>=12 > 35")
    if b15 > 22:
        fails.append("blowout>=15 > 22")
    if avg3 > 0.76:
        fails.append("3-letter ratio > 0.76")
    if best4 < 40:
        fails.append("best4+ < 40")
