  ...
  return (
    <div
      style={{
        ...
        fontFamily:
          "system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI','Hiragino Kaku Gothic ProN','Yu Gothic',sans-serif",
        color: "#0f172a",
      }}
    >
      ...
      <div style={card}>
        ...
        <Step
          n="1"
          title="文字を置く"
          body="緑のマスに1文字を置き、隣り合う文字をつないで単語を作ります。日本語版では2文字語も使えますが、効果は小さめです。"
        />
        <Step
          n="2"
          title="領地を取る"
          body="単語の経路が自分の領地になります。3文字以上、4文字以上になるほど盤面が大きく動きます。"
        />
        <Step
          n="3"
          title="囲む"
          body="相手の領地グループの周囲を塞ぐと、捕獲・反転に近づきます。"
        />
      </div>
      ...
      <Mech
        name="LOCK"
        body="囲った領地はロックされ、相手に奪われにくくなります。守りの拠点です。"
      />

      <Mech
        name="奪字"
        body="敵文字を含む単語を作ると、敵マスを1つ中立化できます。守りを崩す突破口です。"
      />

      <Mech
        name="2×2回転"
        body="1試合1回、2×2の文字だけを回転できます。所有権は動かず、文字配置だけが変わります。"
      />

      <Mech
        name="CUT / BRIDGE"
        body="CUTは相手の連結を分断、BRIDGEは自分の離れた領地を接続します。"
      />

      <Mech
        name="ENCIRCLE / SWING"
        body="包囲や大きな領地変動で、一手で流れが変わります。"
      />

      <Mech
        name="OPEN / Open Sides"
        body="領地グループの周囲に残る、まだ塞がれていない接点です。OP1は包囲寸前、OP2は圧迫、OP3はまだ余裕。単語で相手のOPENを減らすと、次の捕獲・囲みに近づきます。"
      />
