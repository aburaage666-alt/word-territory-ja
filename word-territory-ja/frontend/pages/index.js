// ?? Hand generator ???????????????????????????????????????????????????????????
// WT_JA_HAND_GENERATOR_FINAL_NO_MOJIBAKE_20260606
const VOWELS = "\u3042\u3044\u3046\u3048\u304a".split("");
const CONSONANTS = "\u3042\u3044\u3046\u3048\u304a\u304b\u304d\u304f\u3051\u3053\u3055\u3057\u3059\u305b\u305d\u305f\u3061\u3064\u3066\u3068\u306a\u306b\u306c\u306d\u306e\u306f\u3072\u3075\u3078\u307b\u307e\u307f\u3080\u3081\u3082\u3084\u3086\u3088\u3089\u308a\u308b\u308c\u308d\u308f\u3092\u3093\u304c\u304e\u3050\u3052\u3054\u3056\u3058\u305a\u305c\u305e\u3060\u3062\u3065\u3067\u3069\u3070\u3073\u3076\u3079\u307c\u3071\u3074\u3077\u307a\u307d".split("").filter(c => !VOWELS.includes(c));
...
function sanitizeKanaTile(c) {
  return /^[\u3041-\u3096\u30fc]$/.test(c) ? c : randomLetter(CONSONANTS);
}
...
function replaceCard(hand, usedLetter) {
  const cleanHand = (hand || []).map(sanitizeKanaTile);
  const idx = cleanHand.findIndex(c => c === usedLetter);
...
import {
  ...
  getAlmost, getLetterPreview, getMarket, getSuggestions, getSynergyOptions, selectSynergy, get脅威, createAsyncMatch, getAsyncMatch, submitAsyncMove, seedAsyncMove, passAsyncTurn,
...
// WT_JA_自由_INPUT_AND_UI_FINAL_20260606
const WT_JA_INPUT_KANA_POOL = "\u3042\u3044\u3046\u3048\u304a\u304b\u304d\u304f\u3051\u3053\u3055\u3057\u3059\u305b\u305d\u305f\u3061\u3064\u3066\u3068\u306a\u306b\u306c\u306d\u306e\u306f\u3072\u3075\u3078\u307b\u307e\u307f\u3080\u3081\u3082\u3084\u3086\u3088\u3089\u308a\u308b\u308c\u308d\u308f\u3092\u3093\u304c\u304e\u3050\u3052\u3054\u3056\u3058\u305a\u305c\u305e\u3060\u3062\u3065\u3067\u3069\u3070\u3073\u3076\u3079\u307c\u3071\u3074\u3077\u307a\u307d";
...
const asArray = (value) => {
  ...
  if (
    Array.isArray(value.マス) ||
    Array.isArray(value.path) ||
    Array.isArray(value.threat_マス) ||
    Array.isArray(value.affected_マス) ||
    Array.isArray(value.captured_マス)
  ) return [value];
  return [];
};
  ...
  return asArray(value)
    .map((item) => {
      const マスSource =
        ...
        item?.captured_マス ??
        [];
      const マス = asArray(マスSource).map(toCell).filter(Boolean);
      const mainCell = toCell(item) || toCell(item?.move) || toCell(item?.target) || マス[0] || null;
      return {
        ...(item || {}),
        row: mainCell?.row,
        col: mainCell?.col,
    ...
    .filter((item) => {
      ...
      const hasCells = Array.isArray(item.マス) && item.マス.length > 0;
...
function wtJaTo脅威List(value) {
  ...
  return raw.map((item) => {
    ...
    const マスRaw = wtJaToArray(obj.マス || obj.path || obj.threat_マス || obj.affected_マス);
...
function terrainComboLabel(label, move = null) {
  ...
  if (raw === "CAPTURE" && move?.captureCount) return `Captured ${move.captureCount} マス${move.captureCount === 1 ? "" : "s"}`;
  if (raw === "DOUBLE CAPTURE" && move?.captureCount) return `Double Capture (+${move.captureCount} マス)`;
...
function moveInsightLines(m) {
  ...
  if ((m.captureCount || 0) > 0) lines.push(`Captured ${m.captureCount} マス${m.captureCount === 1 ? "" : "s"}.`);
  if ((m.fortifiedCellsGained || 0) > 0) lines.push(`Locked ${m.fortifiedCellsGained} マス${m.fortifiedCellsGained === 1 ? "" : "s"}.`);
  const labels = (m.comboLabels || []).map(x => terrainComboLabel(x, m));
...
function buildEmojiBoard(board) {
  ...
  return board.map(row =>
    row.map(マス => {
      if (!マス.letter) return "⬜";
      if (マス.owner === "RED") return "🟥";
      if (マス.owner === "BLUE") return "🟦";
      return "⬜";
    }).join("")
  ).join("\n");
}
...
// ── Cell ──────────────────────────────────────────────────────────────────────
function Cell({ マス, sel, placed, legal, changed, captured, lockedNow, bridgePath, lockNeighbor, tutorialPlace, tutorialPath, disabled, gen, attack, inPath, threat, threatMove, captureOrder, lockOrder, onClick }) {
  const cls = ["マス",
    マス.owner === "RED" ? "cr" : マス.owner === "BLUE" ? "cb" : "",
    マス.fortified ? "ft" : "", sel ? "sl" : "", placed ? "pl" : "",
    legal ? "lg" : "", disabled && !sel ? "dm" : "",
    attack ? "atk" : "",   // opponent マス that can be attacked
    threat ? "threat" : "", threatMove ? "threatMove" : "",
    bridgePath ? "bridge-path" : "", lockNeighbor ? "lock-neighbor" : "",
    tutorialPlace ? "tut-pulse tut-マス" : "", tutorialPath ? "tut-arrow-マス" : "",
    inPath ? "inpath" : "", // opponent マス currently in selected path (will be captured)
  ].filter(Boolean).join(" ");
  const animStyle = {
    "--cap-order": captureOrder != null ? captureOrder : 0,
    "--cap-delay": captureOrder != null ? `${captureOrder * 80}ms` : "0ms",
    "--my-color": マス.owner === "RED" ? "rgba(220,38,38,.42)" : マス.owner === "BLUE" ? "rgba(37,99,235,.42)" : "#fafafa",
    "--opp-color": マス.owner === "RED" ? "rgba(37,99,235,.36)" : マス.owner === "BLUE" ? "rgba(220,38,38,.36)" : "#fafafa",
  ...
  return (
    <button className={cls} onClick={onClick} style={animStyle} disabled={disabled}
      ...
      {マス.letter || ""}
      {マス.fortified && <span className="lock-shield" title="Fortified ground">🛡</span>}
...
// ── Main ──────────────────────────────────────────────────────────────────────
// WT_JA_FREE_INPUT_AND_UI_FINAL_20260606
const WT_JA_INPUT_KANA_POOL = "\u3042\u3044\u3046\u3048\u304a\u304b\u304d\u304f\u3051\u3053\u3055\u3057\u3059\u305b\u305d\u305f\u3061\u3064\u3066\u3068\u306a\u306b\u306c\u306d\u306e\u306f\u3072\u3075\u3078\u307b\u307e\u307f\u3080\u3081\u3082\u3084\u3086\u3088\u3089\u308a\u308b\u308c\u308d\u308f\u3092\u3093\u304c\u304e\u3050\u3052\u3054\u3056\u3058\u305a\u305c\u305e\u3060\u3062\u3065\u3067\u3069\u3070\u3073\u3076\u3079\u307c\u3071\u3074\u3077\u307a\u307d";
...
export default function Home() {
  ...
  const [submitted,   set確定ted]   = useState(false);
  ...
  function reset() {
    ...
    set確定ted(false); summaryFired.current = false;
  }
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!canマスed) setError(e.message || "Bot failed");
      }
      if (!canマスed) setThinking(false);
    };
    run();
    return () => { canマスed = true; setThinking(false); };
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!canマスed) setError(e.message || "Spectator demo failed");
      }
      if (!canマスed) setThinking(false);
    };
    run();
  ...
  // Auto-focus letter input when マス is placed
  useEffect(() => {
  ...
  // Opponent マス adjacent to any placeable empty マス = attackable
  ...
  const attackableSet = useMemo(() => {
    ...
    for (let r = 0; r < BS; r++) {
      for (let c = 0; c < BS; c++) {
        const マス = state?.board?.[r]?.[c];
    if (!マス) return;
        if (マス.letter && マス.owner === opponent && !マス.fortified) {
  ...
  // Opponent マス currently in the selected path (will be captured if submitted)
  const inPathOpponentSet = useMemo(() => {
    if (!path.length) return new Set();
    const s = new Set();
    path.forEach(p => {
      const マス = state?.board?.[p.row]?.[p.col];
      if (マス?.owner === opponent) s.add(asKey(p.row, p.col));
    });
    return s;
  }, [path, state?.turn]);
  const hasNbr = (r,c) => {
  ...
  const isDim = (r,c) => {
    ...
    const マス = state?.board?.[r]?.[c];
    if (!マス) return true;
    // Already selected マス are not dim but not clickable again
    if (isSel(r,c)) return false;
    // Phase 0: nothing selected yet
    if (path.length === 0) {
      // Can start from a green マス (will become placed) OR existing letter
      return !isLegal(r,c) && !マス.letter;
    }
    // Must be adjacent to last マス in path
    const last = path[path.length - 1];
    if (!adj(last, {row:r, col:c})) return true;
    // Can select: existing letter OR the green placed マス
    if (マス.letter) return false;
    if (isLegal(r,c) && !placed) return false; // green マス not yet set as placed
  ...
  function clickCell(r,c) {
    ...
    // Deselect last マス if tapping it again (undo last step)
    if (path.length > 0 && path[path.length-1].row===r && path[path.length-1].col===c) {
      const newPath = path.slice(0, -1);
      setPath(newPath);
      // If we removed the placed マス from path, unset placed
    ...
    if (isSel(r,c)) return; // already in path (not last マス)

    // Phase 0: start path
    if (path.length === 0) {
      if (isLegal(r,c)) {
        // Green マス → becomes placed マス
        setPlaced({row:r, col:c});
        setPath([{row:r, col:c}]);
        setError("");
      } else if (マス.letter) {
        // Existing letter → start of path (placed マス comes later)
        setPath([{row:r, col:c}]);
        setError("");
      }
      return;
    ...
    // Adding green マス (placed マス not yet set)
    ...
    // Adding the already-set placed マス
    if (placed && placed.row===r && placed.col===c) {
      setPath(prev => [...prev, {row:r, col:c}]);
    }
  }
  ...
  async function seed() {
    if (!placed) { setError("Tap a green square first."); return; }
    if (!letter) { setError("ひらがな1文字 in the input box."); return; }
    try {
  ...
  const boardBannerText = lastMoveIsSwing && lastMove ? [
    (lastMove.captureCount||0)>0 ? `Captured ${lastMove.captureCount} マス${lastMove.captureCount===1?"":"s"}` : null,
    (lastMove.comboLabels||[]).some(x=>String(x).includes("BRIDGE")) ? "Bridge connected zones" : null,
    (lastMove.fortifiedCellsGained||0)>0 ? `Locked ${lastMove.fortifiedCellsGained} マス${lastMove.fortifiedCellsGained===1?"":"s"}` : null,
  ...
  return <>
    ...
    <main className="page">
      {showIntro && (
        <div className="intro-bg">
          <div className="intro-card">
            ...
            <p>Watch letters claim ground, trigger captures, lock マス, and reshape the map.</p>
            <div className="intro-steps">
              <span>1. Choose a letter</span>
              <span>2. Tap a glowing マス</span>
      ...
      <div className="sbar">
        <div className="srow">
          <span className="stxt red-t">🔴 {redT} マス</span>
          <span className="smid">
            {redT===blueT ? "同点" : `${redT>blueT?"🔴 RED":"🔵 BLUE"} +${Math.abs(redT-blueT)}`}
          </span>
          <span className="stxt blue-t">{blueT} マス 🔵</span>
        </div>
      ...
      {showルール&&(
        <div className="rules">
          ...
          <ol>
            ...
            <li>Enclose opponent マス to <strong>capture</strong> them. Surrounded own マス become 🏰 <strong>Fortified</strong>.</li>
            ...
            <li><strong>Goal:</strong> More red マス than blue wins. Territory beats vocabulary.</li>
        ...
        <div className="bcol">
          ...
          <div className="bwrap">
            ...
            <div className="board-wrap"><div className={`board ${boardOpeningClass} ${spectatorMode ? "board-demo" : ""} ${lastMoveIsSwing ? "board-swing" : ""} ${bridgeFlash ? "board-bridge" : ""}`}>
              {state.board.map(row=>row.map(マス=>{
                const k=asKey(マス.row,マス.col);
                const vp = Array.isArray(valuePrev)
                  ? valuePrev.find(p => p.row === マス.row && p.col === マス.col)
                  : null;
                const showVp = !state.winner && vp && !マス.letter && ((Number(vp.gain)||0) >= 2 || (vp.roles||[]).length > 0 || !!vp.word);
                return <div key={k} className="マス-slot">
                  <Cell マス={マス}
                    sel={isSel(マス.row,マス.col)} placed={placed?.row===マス.row&&placed?.col===マス.col}
                    legal={!placed&&isLegal(マス.row,マス.col)}
                    ...
                    disabled={isDim(マス.row,マス.col)} gen={animGen}
                    attack={attackableSet.has(k) && !isSel(マス.row,マス.col)}
                    threat={threatCellSet.has(k)} threatMove={threatMoveSet.has(k)}
                    inPath={inPathOpponentSet.has(k)}
                    onClick={()=>clickCell(マス.row,マス.col)}/>
            ...
            <>
              ...
              <div className="battle-report-card report-polished">
                ...
                <div className="report-stats">
                  <span>Largest capture: {Math.max(0, ...((state.move履歴||[]).map(m=>m.captureCount||0)))} マス</span>
  ...
  <div className="freeInput">
    ...
    <button
      ...
      disabled={!human() || !wtJaHasKana(freeLetter)}
      onClick={() => {
        const chosen = wtJaNormalizeKanaInput(freeLetter);
        if (!chosen) {
          setError("????1????????????");
...
{/* move controls */}
          {!state.winner && <div className="mpanel">
              ...
              <div className={`pvbox ${ok?"pvok":""}`}>
                ...
                {preview?(
                  ...
                  <div className="pvhint">
                    {!placed
                      ? (market.active.length > 0 ? (thinking ? "Bot is thinking..." : state?.winner ? "Battle Report" : "Choose a tile from 文字カード above ↑ then read the Territory Preview.") : "Tap a green square to place a letter.")
                      : !letter
                      ? "ひらがな1文字を入力してください。"
                      : path.length < 2
            ...
            <div className="brow">
              <button className={`ba bsubmit ${showTutorial && tutorialStep===3 ? "tut-pulse tut-submit" : ""}`} onClick={submit} disabled={!human()}>{showTutorial && tutorialStep===3 ? "Capture Word ⚔" : ok ? "領地を取る ⚔" : "確定"}</button>
          ...
          <div className="panel panel-suggest">
            ...
            {showSuggest&&(
              <div className="chips sc">
                {suggestionList.length ? suggestionList.map(w=><span key={w} className="chip">{w}</span>):<div className="no-word-hint">使える単語が見つかりません。<br/>Use <strong>Seed</strong> to place a tile without capturing.</div>}
              </div>
            )}
          </div>
              ...
              <div className="threat-list">
                {threatList && threatList.length ? threatList.slice(0,5).map((t,i)=>(
                  <div className="threat-row" key={i}>
                    ...
                    <span>{(t.マス||[]).length} マス{(t.マス||[]).length===1?"":"s"} exposed</span>
                  </div>
                )) : <div className="muted">直近の脅威はありません。</div>}
              </div>
            )}
          </div>
<div className="panel panel-history">
            ...
            {show履歴&&(
              <div className="hist" ref={histRef}>
                {!state.move履歴.length&&<div className="muted">No moves yet</div>}
          ...
          <div className="modal">
            {dailyMode&&dailyInfo?(
              <>
                ...
                <div className="scard">
                  <div className="scrow"><span>🔴 YOU</span><strong>{redT} マス</strong></div>
                  <div className="scrow"><span>🔵 BOT</span><strong>{blueT} マス</strong></div>
              ...
              <>
                ...
                <div className="scard">
                  <div className="scrow"><span>🔴 RED</span><strong>{redT} マス</strong></div>
                  <div className="scrow"><span>🔵 BLUE</span><strong>{blueT} マス</strong></div>
                </div>
                {bestMove && <div className="best-move-card"><strong>Best Territorial Swing:</strong> {moveLabel(bestMove)}</div>}
                {topMoves.length>0&&<><h3>Top Territorial Swings</h3>{topMoves.map((m,i)=><HistItem key={i} m={m}/>)}</>}
                <div className="modal-btns">
    ...
    <style jsx global>{`
      *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
      ...
      .board{--マス:52px;--gap:6px;position:relative;display:grid;grid-template-columns:repeat(7,var(--マス));gap:var(--gap);justify-content:center;min-width:max-content;padding:12px;border-radius:20px;background:
      ...
      .board-demo .マス{opacity:1!important;filter:none!important}
      .マス-slot{position:relative;width:var(--マス);height:var(--マス);display:flex;align-items:center;justify-content:center}
      .マス{position:relative;width:var(--マス);height:var(--マス);border:1px solid #d8d2c4;border-radius:12px;background:#f7f3ea;font-size:19px;font-weight:900;cursor:pointer;transition:background .16s, transform .12s, box-shadow .16s,color .16s;color:#1f2933;box-shadow:inset 0 1px 0 rgba(255,255,255,.65),0 2px 7px rgba(31,41,51,.10);font-feature-settings:"kern" 1;user-select:none}
      .マス.cr{background:rgba(216,92,92,.28);border-color:rgba(216,92,92,.58);color:#7a2424;box-shadow:inset 0 1px 0 rgba(255,255,255,.52),0 2px 8px rgba(216,92,92,.12)}
      .マス.cb{background:rgba(79,131,204,.30);border-color:rgba(79,131,204,.58);color:#173e74;box-shadow:inset 0 1px 0 rgba(255,255,255,.52),0 2px 8px rgba(79,131,204,.13)}
      .マス.ft{border:2px solid rgba(17,24,39,.78);box-shadow:inset 0 0 0 3px rgba(17,24,39,.18),inset 0 1px 0 rgba(255,255,255,.55),0 2px 7px rgba(17,24,39,.16)}
      .マス.sl{outline:3px solid #f59e0b;outline-offset:-3px}
      .マス.pl{box-shadow:inset 0 0 0 3px #111827,0 2px 7px rgba(31,41,51,.12)}
      .マス.lg{background:#e9f9e7;border-color:#70bf6f;box-shadow:0 0 0 3px rgba(112,191,111,.12),0 2px 7px rgba(31,41,51,.08)}
      .マス:hover:not(.dm):not(.sl){transform:translateY(-1px);filter:brightness(1.04)}
      .マス.cr:hover:not(.dm){background:rgba(216,92,92,.40)}
      .マス.cb:hover:not(.dm){background:rgba(79,131,204,.42)}
      .マス.lg:hover{background:#dff5dc;transform:translateY(-1px)}
      .マス.dm{opacity:.72;cursor:not-allowed}
      .マス:not(.cr):not(.cb):not(.lg):not([data-letter])::after{
      ...
      .マス[data-chg]{animation:aclaim 500ms ease forwards}
      .マス[data-cap]{animation:acap 1050ms cubic-bezier(.2,.8,.2,1) forwards;animation-delay:calc(var(--cap-order,0) * 90ms);animation-fill-mode:both}
      .マス[data-lk]{animation:alk 850ms ease forwards;animation-delay:var(--lock-delay,0ms);animation-fill-mode:both}
      ...
      .マス.threat{box-shadow:inset 0 0 0 2px rgba(37,99,235,.48),0 0 0 3px rgba(37,99,235,.08);background:rgba(37,99,235,.08)}
      .マス.threatMove{outline:2px dashed rgba(37,99,235,.42);outline-offset:-5px}
      ...
      .マス.bridge-path{animation:bridgeGlow 1050ms ease-out 1 forwards}
      .マス.lock-neighbor{animation:lockNeighborPulse 900ms ease-in-out 1}
      ...
      .tut-arrow-マス::after,.tut-マス::after{content:"";position:absolute;left:50%;top:-9px;transform:translateX(-50%);border-left:6px solid transparent;border-right:6px solid transparent;border-top:8px solid #111827;z-index:44}
      ...
      /* attack highlighting */
      .マス{position:relative}
      .マス.atk{box-shadow:inset 0 0 0 2px rgba(255,140,0,.8);background:rgba(255,140,0,.06)}
      .マス.inpath{box-shadow:inset 0 0 0 3px #e65c00 !important;background:rgba(255,100,0,.25) !important;animation:ainpath .5s ease infinite alternate}
      ...
      @media(max-width:900px){
        ...
        .マス-slot,.マス{width:44px;height:44px;font-size:15px;border-radius:7px}
        .bwrap{padding:10px}
      ...
      @media(min-width:901px){
        ...
        .board{
          --マス:56px;
          --gap:7px;
          grid-template-columns:repeat(7,var(--マス));
          gap:var(--gap);
          padding:12px;
        }
        .マス-slot,.マス{width:var(--マス);height:var(--マス)}
      ...
      @media(max-width:900px){
        ...
        .board{--マス:min(10.6vw,52px);--gap:5px;grid-template-columns:repeat(7,var(--マス))!important;gap:var(--gap);padding:9px}
        .マス-slot,.マス{width:var(--マス)!important;height:var(--マス)!important}
      ...
      @media(max-width:600px){
        ...
        .board{
          --マス:calc((100vw - 48px) / 7);
          --gap:4px;
          grid-template-columns:repeat(7,var(--マス))!important;
          width:max-content;
          max-width:100%;
          gap:var(--gap);
          padding:7px;
        ...
        .マス-slot,.マス{
          width:var(--マス)!important;
          height:var(--マス)!important;
      ...
      @media(max-width:360px){
        .board{--マス:calc((100vw - 36px) / 7);--gap:3px}
