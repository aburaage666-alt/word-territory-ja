...
async function request(path,options={}){
  // WT_INTRO_TUTOR_REQUEST_HARDENING_V2
  try {
    ...
    if (method === "POST" && String(path || "") === "/games") {
      ...
      const bm = raw.includes("intro") || raw.includes("tutorial") || raw.includes("learn") || raw.includes("practice")
        ? "intro"
        : (raw.includes("quick") || raw.includes("5") ? "quick5" : "standard");
      const size = bm === "standard" ? 7 : 5;
      body.boardMode = bm;
      body.board_mode = bm;
      body.boardSize = size;
      body.board_size = size;
      if (bm === "intro") body.botLevel = "easy";
