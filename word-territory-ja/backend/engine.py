...
def validate_and_apply_move(state: GameState, row: int, col: int, letter: str, path, advance_market_flag: bool = False, dazi: bool = False):
    ...
    # WT_MULTI_IMPACT_CALL_V4
    for _wt_mi_label in _wt_multi_impact_labels_v4(before, temp, player, word, delta, combos):
...
# WT_JA_ATTACHED_BOT_TUNING_SAFE_V1
# Safe wrapper distilled from attached engine.py.
# Goals:
# - Easy bot: beginner-friendly, avoids tactical crushing.
# - Normal bot: active but beatable, does not always select maximum move.
# - Market: consumed tile only is replaced; no full reshuffle feeling.
# Strong bot remains unchanged.

def _wt_ja_attached_is_ja_v1():
    try:
        return globals().get("_LANG") == "ja"
    except Exception:
        return False
...
def _wt_ja_attached_is_kana_v1(x):
    if x == "*":
        return True
    if not isinstance(x, str) or len(x) != 1:
        return False
...
def _wt_ja_attached_pool_v1():
    try:
        pool = [x for x in _ALL_LETTERS if _wt_ja_attached_is_kana_v1(x) and x != "*"]
        if pool:
            return pool
    except Exception:
        pass
...
def _wt_ja_attached_fill_letter_v1(existing=None, offset=0):
    existing = set(existing or [])
    pool = _wt_ja_attached_pool_v1()
    for i in range(len(pool)):
...
def _wt_ja_attached_clean_market_v1(seq, existing=None, allow_wild=True, offset=0):
    if not _wt_ja_attached_is_ja_v1():
        return seq
    ...
    for x in seq or []:
        if x == "*" and allow_wild and "*" not in out:
            out.append(x)
        elif _wt_ja_attached_is_kana_v1(x) and x != "*" and x not in out:
            out.append(x)

    while len(out) < 3:
        out.append(_wt_ja_attached_fill_letter_v1(existing | set(out), offset + len(out)))

    return out[:3]


try:
    _wt_ja_attached_orig_advance_market_v1 = advance_market

    def advance_market(state, used_letter):
        if not _wt_ja_attached_is_ja_v1():
            return _wt_ja_attached_orig_advance_market_v1(state, used_letter)

        active = _wt_ja_attached_clean_market_v1(
            getattr(state, "marketLetters", []) or [],
            allow_wild=True,
            offset=0,
        )
        preview = _wt_ja_attached_clean_market_v1(
            getattr(state, "previewLetters", []) or [],
            existing=set(active),
            allow_wild=False,
            offset=7,
        )
        ...
        while len(active) < 3:
            active.append(_wt_ja_attached_fill_letter_v1(active, len(active)))

        while len(preview) < 3:
            preview.append(_wt_ja_attached_fill_letter_v1(set(active) | set(preview), 7 + len(preview)))
        ...
        replacement = preview[0] if preview else _wt_ja_attached_fill_letter_v1(new_active, 11)
        new_active[replace_idx] = replacement

        new_preview = [x for x in preview[1:] if x not in new_active and _wt_ja_attached_is_kana_v1(x)]
        while len(new_preview) < 3:
            new_preview.append(_wt_ja_attached_fill_letter_v1(set(new_active) | set(new_preview), 17 + len(new_preview)))
...
def _wt_ja_attached_move_score_v1(state, move, player, easy=False):
    try:
        ns = simulate_move(state, move)
        last = ns.moveHistory[-1]
        labels = set(last.comboLabels or [])
        ...
        score = 0.0
        score += word_score(word) * (0.70 if easy else 0.90)
        score += len(move.get("path", []) or []) * 0.20
        score += territory * (0.18 if easy else 0.42)

        if n <= 2:
            score -= 4.0
        elif n == 3:
            score += 0.4 if easy else 0.2
        elif n == 4:
            score += 0.2 if easy else 0.7
        elif n >= 5:
            score -= 3.5 if easy else 1.4
...
try:
    _wt_ja_attached_orig_choose_bot_move_v1 = choose_bot_move

    def choose_bot_move(state):
        if not _wt_ja_attached_is_ja_v1():
            return _wt_ja_attached_orig_choose_bot_move_v1(state)
        ...
        scored = []
        for m in moves:
            scored.append((_wt_ja_attached_move_score_v1(state, m, player, easy=easy), m))
        ...
        if easy:
            # Beginner bot: avoid best move. Prefer lower-middle playable moves.
            idx = min(len(scored) - 1, max(0, (len(scored) * 2) // 3))
        else:
            if lead >= 4:
                # If bot/current player is already ahead, soften.
                idx = min(len(scored) - 1, max(0, len(scored) // 2))
            elif lead <= -6:
                # If losing badly, allow a better move.
                idx = min(len(scored) - 1, max(0, len(scored) // 4))
            else:
                # Normal: active but not maximizer.
...
_SECOND_PLAYER_KOMI = 6.0  # WT_SECOND_PLAYER_KOMI_6_FINAL_OVERRIDE_V1
...
# WT_JA_CHOONPU_ENGINE_RULES_V1_BEGIN
# Choonpu / long vowel mark rules:
# 1. "ー" never appears in Letter Market.
# 2. "ー" may be chosen through Free Letter.
# 3. "ー" is legal only if the submitted path forms a valid word.
# 4. Seed with "ー" is forbidden.
# 5. Words cannot start or end with "ー".

_WT_JA_CHOONPU_V1 = "ー"

try:
    if globals().get("_LANG") == "ja":
        if _WT_JA_CHOONPU_V1 not in _ALL_LETTERS:
            _ALL_LETTERS.append(_WT_JA_CHOONPU_V1)
        try:
            _LETTER_WEIGHTS[_WT_JA_CHOONPU_V1] = 0
        except Exception:
            pass
except Exception:
    pass

def _wt_ja_choon_is_ja_v1():
    try:
        return globals().get("_LANG") == "ja"
    except Exception:
        return False

def _wt_ja_choon_is_kana_market_tile_v1(x):
    if x == "*":
        return True
    if not isinstance(x, str) or len(x) != 1:
        return False
    if x == _WT_JA_CHOONPU_V1:
        return False
    o = ord(x)
    return 0x3041 <= o <= 0x3096

def _wt_ja_choon_pool_v1():
    try:
        pool = [x for x in _ALL_LETTERS if _wt_ja_choon_is_kana_market_tile_v1(x) and x != "*"]
        if pool:
            return pool
    except Exception:
        pass
...
def _wt_ja_choon_fill_v1(existing=None, offset=0):
    existing = set(existing or [])
    pool = _wt_ja_choon_pool_v1()
    for i in range(len(pool)):
        c = pool[(i + offset) % len(pool)]
        if c not in existing:
            return c
...
def _wt_ja_choon_clean_seq_v1(seq, existing=None, allow_wild=True, offset=0):
    if not _wt_ja_choon_is_ja_v1():
        return seq
    existing = set(existing or [])
    out = []
    for x in seq or []:
        if x == "*" and allow_wild and "*" not in out:
            out.append(x)
        elif _wt_ja_choon_is_kana_market_tile_v1(x) and x not in out:
            out.append(x)
    while len(out) < 3:
        out.append(_wt_ja_choon_fill_v1(existing | set(out), offset + len(out)))
    return out[:3]

def _wt_ja_choon_clean_pair_v1(pair):
    if not _wt_ja_choon_is_ja_v1():
        return pair
    try:
        active, preview = pair
        ...
        return pair
    active2 = _wt_ja_choon_clean_seq_v1(active, allow_wild=True, offset=0)
    preview2 = _wt_ja_choon_clean_seq_v1(preview, existing=set(active2), allow_wild=False, offset=7)
    return active2, preview2

try:
    _wt_ja_choon_orig_generate_letter_market_v1 = generate_letter_market
    def generate_letter_market(state):
        return _wt_ja_choon_clean_pair_v1(_wt_ja_choon_orig_generate_letter_market_v1(state))
except Exception:
    pass

try:
    _wt_ja_choon_orig_advance_market_v1 = advance_market
    def advance_market(state, used_letter):
        return _wt_ja_choon_clean_pair_v1(_wt_ja_choon_orig_advance_market_v1(state, used_letter))
except Exception:
    pass

try:
    _wt_ja_choon_orig_apply_seed_move_v1 = apply_seed_move
    def apply_seed_move(state, row: int, col: int, letter: str, advance_market_flag: bool = False):
        if _wt_ja_choon_is_ja_v1() and _norm_letter(letter) == _WT_JA_CHOONPU_V1:
            raise ValueError("のばし棒「ー」は種まきでは使えません。単語が成立する通常手でのみ使えます。")
        return _wt_ja_choon_orig_apply_seed_move_v1(state, row, col, letter, advance_market_flag=advance_market_flag)
except Exception:
    pass

try:
    _wt_ja_choon_orig_validate_path_and_word_v1 = validate_path_and_word
    def validate_path_and_word(state, row: int, col: int, letter: str, path):
        word = _wt_ja_choon_orig_validate_path_and_word_v1(state, row, col, letter, path)
        if _wt_ja_choon_is_ja_v1() and _WT_JA_CHOONPU_V1 in str(word or ""):
            if str(word).startswith(_WT_JA_CHOONPU_V1) or str(word).endswith(_WT_JA_CHOONPU_V1):
...
try:
    _wt_ja_choon_orig_fast_bot_moves_v1 = _fast_bot_moves
    def _fast_bot_moves(state, max_len, max_results, excluded):
        moves = _wt_ja_choon_orig_fast_bot_moves_v1(state, max_len, max_results, excluded)
        if not _wt_ja_choon_is_ja_v1():
            return moves
        return [m for m in moves if m.get("letter") != _WT_JA_CHOONPU_V1]
except Exception:
    pass

def wt_ja_choonpu_rules_summary():
    return {
        "marketIncludesChoonpu": False,
        "freeInputAllowsChoonpu": True,
        "seedAllowsChoonpu": False,
        "wordStartEndAllowsChoonpu": False,
        "normalValidWordMoveAllowsChoonpu": True,
    }
# WT_JA_CHOONPU_ENGINE_RULES_V1_END

# WT_JA_CHOONPU_FINAL_NORMALIZER_V2_BEGIN
# Final choonpu normalizer.
# Reason:
# The first choonpu patch allowed ー in profile/market guards, but engine-level
# _norm_letter may still normalize it away through dictionary.normalize_word.
# This final layer guarantees:
# - Katakana -> Hiragana
# - ー is preserved for normal word moves
# - ー is included in _ALL_LETTERS for validation
# - ー is still excluded from market and seed

import unicodedata as _wt_ja_choon_unicodedata_v2

_WT_JA_CHOONPU_FINAL_V2 = "ー"

try:
    _wt_ja_prev_norm_word_v2 = _norm_word
except Exception:
    _wt_ja_prev_norm_word_v2 = None

try:
    _wt_ja_prev_norm_letter_v2 = _norm_letter
except Exception:
    _wt_ja_prev_norm_letter_v2 = None

def _wt_ja_choon_normalize_text_v2(value):
    txt = _wt_ja_choon_unicodedata_v2.normalize("NFKC", str(value or "")).strip()
    out = []
    for ch in txt:
        code = ord(ch)
        # Katakana -> Hiragana
...
def _norm_word(w: str) -> str:
    if globals().get("_LANG") == "ja":
        return _wt_ja_choon_normalize_text_v2(w)
    if _wt_ja_prev_norm_word_v2:
        return _wt_ja_prev_norm_word_v2(w)
    return str(w or "").upper().strip()

def _norm_letter(ch: str) -> str:
    if globals().get("_LANG") == "ja":
        w = _wt_ja_choon_normalize_text_v2(str(ch or "")[:10])
        return w[:1] if w else ""
    if _wt_ja_prev_norm_letter_v2:
        return _wt_ja_prev_norm_letter_v2(ch)
    w = _norm_word(str(ch or "")[:1])
    return w[:1] if w else ""

try:
    if globals().get("_LANG") == "ja":
        if _WT_JA_CHOONPU_FINAL_V2 not in _ALL_LETTERS:
            _ALL_LETTERS.append(_WT_JA_CHOONPU_FINAL_V2)
        try:
            _LETTER_WEIGHTS[_WT_JA_CHOONPU_FINAL_V2] = 0
        except Exception:
            pass
except Exception:
    pass

def _wt_ja_choon_final_is_market_tile_v2(x):
    if x == "*":
        return True
    if not isinstance(x, str) or len(x) != 1:
        return False
    if x == _WT_JA_CHOONPU_FINAL_V2:
        return False
    o = ord(x)
    return 0x3041 <= o <= 0x3096

def _wt_ja_choon_final_pool_v2():
    try:
        pool = [x for x in _ALL_LETTERS if _wt_ja_choon_final_is_market_tile_v2(x) and x != "*"]
        if pool:
            return pool
    except Exception:
        pass
...
def _wt_ja_choon_final_clean_seq_v2(seq, existing=None, allow_wild=True, offset=0):
    if globals().get("_LANG") != "ja":
        return seq
    existing = set(existing or [])
    out = []
    for x in seq or []:
        if x == "*" and allow_wild and "*" not in out:
            out.append(x)
        elif _wt_ja_choon_final_is_market_tile_v2(x) and x not in out:
            out.append(x)
    pool = _wt_ja_choon_final_pool_v2()
    i = offset
    while len(out) < 3:
        c = pool[i % len(pool)]
        i += 1
...
def _wt_ja_choon_final_clean_pair_v2(pair):
    if globals().get("_LANG") != "ja":
        return pair
    try:
        active, preview = pair
        ...
        return pair
    active2 = _wt_ja_choon_final_clean_seq_v2(active, allow_wild=True, offset=0)
    preview2 = _wt_ja_choon_final_clean_seq_v2(preview, existing=set(active2), allow_wild=False, offset=7)
    return active2, preview2

try:
    _wt_ja_choon_final_orig_generate_letter_market_v2 = generate_letter_market
    def generate_letter_market(state):
        return _wt_ja_choon_final_clean_pair_v2(_wt_ja_choon_final_orig_generate_letter_market_v2(state))
except Exception:
    pass

try:
    _wt_ja_choon_final_orig_advance_market_v2 = advance_market
    def advance_market(state, used_letter):
        return _wt_ja_choon_final_clean_pair_v2(_wt_ja_choon_final_orig_advance_market_v2(state, used_letter))
except Exception:
    pass

try:
    _wt_ja_choon_final_orig_apply_seed_move_v2 = apply_seed_move
    def apply_seed_move(state, row: int, col: int, letter: str, advance_market_flag: bool = False):
        if globals().get("_LANG") == "ja" and _norm_letter(letter) == _WT_JA_CHOONPU_FINAL_V2:
            raise ValueError("のばし棒「ー」は種まきでは使えません。単語が成立する通常手でのみ使えます。")
        return _wt_ja_choon_final_orig_apply_seed_move_v2(state, row, col, letter, advance_market_flag=advance_market_flag)
except Exception:
    pass

try:
    _wt_ja_choon_final_orig_validate_path_and_word_v2 = validate_path_and_word
    def validate_path_and_word(state, row: int, col: int, letter: str, path):
        word = _wt_ja_choon_final_orig_validate_path_and_word_v2(state, row, col, letter, path)
        if globals().get("_LANG") == "ja" and _WT_JA_CHOONPU_FINAL_V2 in str(word or ""):
            if str(word).startswith(_WT_JA_CHOONPU_FINAL_V2) or str(word).endswith(_WT_JA_CHOONPU_FINAL_V2):
                raise ValueError("のばし棒「ー」は語頭・語尾では使えません。")
        return word
except Exception:
    pass

def wt_ja_choonpu_final_summary_v2():
    return {
        "normLetterChoonpu": _norm_letter("ー"),
        "normHa": _norm_letter("HA"),
        "normKatakana": _norm_letter("カ"),
        "choonpuInAllLetters": _WT_JA_CHOONPU_FINAL_V2 in _ALL_LETTERS,
        "choonpuMarketAllowed": False,
...
# WT_INTELLIGIBILITY_CORE_V3_VERIFIED_BEGIN
# Core 5x5 makes the decision grammar visible:
#   WORD path -> territory; ENCLOSURE -> capture.
# Advanced actions are folded: LOCK / Dazi / 2x2 rotation / Relief swap.
# Standard 7x7 remains competitive mode.

def _wt_core_v3_mode_token(value=""):
    return str(value or "").lower().replace("-", "").replace("_", "").replace("×", "x").strip()

def _wt_core_v3_quick_requested(args=(), kwargs=None, state=None):
    kwargs = kwargs or {}
    try:
        mode = _wt_core_v3_mode_token(kwargs.get("board_mode", kwargs.get("boardMode", "")))
        if mode in ("quick", "quick5", "quick5x5", "5", "5x5", "core", "beginner"):
            return True
    except Exception:
        pass
    ...
    try:
        if state is not None:
            ...
            if int(getattr(state, "boardSize", 0) or 0) == 5:
                return True
            ss = dict(getattr(state, "synergyState", {}) or {})
            if _wt_core_v3_mode_token(ss.get("_boardMode", "")) in ("quick", "quick5", "quick5x5", "5x5", "core"):
                return True
    except Exception:
        pass
    return False
...
def _core_folds(state) -> bool:
    return _wt_core_v3_quick_requested(state=state)

try:
    _wt_core_v3_orig_build_initial_state = build_initial_state

    def build_initial_state(*args, **kwargs):
        quick = _wt_core_v3_quick_requested(args, kwargs)

        try:
            state = _wt_core_v3_orig_build_initial_state(*args, **kwargs)
        except TypeError:
            clean = dict(kwargs)
            clean.pop("board_mode", None)
            clean.pop("boardMode", None)
            clean.pop("board_size", None)
            clean.pop("boardSize", None)
            state = _wt_core_v3_orig_build_initial_state(*args, **clean)

        quick = quick or _wt_core_v3_quick_requested(state=state)
...
try:
    _wt_core_v3_orig_apply_locks = apply_locks
    ...
    def apply_locks(state):
        if _core_folds(state):
            try:
                ...
                pass
            return
        return _wt_core_v3_orig_apply_locks(state)
except Exception:
    pass

def _wt_core_v3_market_has_word_move(state) -> bool:
    if not _core_folds(state):
        return False
    try:
        active = list(getattr(state, "marketLetters", []) or [])
    ...
    for letter in active:
        try:
            ...
            if not l:
                continue
            try:
                if get_letter_preview_moves(state, l, limit=1):
                    return True
            except Exception:
                pass
            try:
                if _fast_bot_moves_for_letter(
                    state,
                    l,
                    max_results=1,
                    excluded=set(getattr(state, "usedWords", []) or [])
...
try:
    _wt_core_v3_orig_apply_seed_move = apply_seed_move

    def apply_seed_move(state, row: int, col: int, letter: str, advance_market_flag: bool = False):
        if _core_folds(state) and _wt_core_v3_market_has_word_move(state):
            raise ValueError("Core 5x5: Seed is rescue-only. Use it only when no active letter can make a word.")
        return _wt_core_v3_orig_apply_seed_move(state, row, col, letter, advance_market_flag=advance_market_flag)
except Exception:
    pass

try:
    _wt_core_v3_orig_apply_dazi_move = apply_dazi_move

    def apply_dazi_move(state, path):
        if _core_folds(state):
            raise ValueError("Core 5x5 folds Disarm/Dazi. Use an ordinary word path.")
        return _wt_core_v3_orig_apply_dazi_move(state, path)
except Exception:
    pass

try:
    _wt_core_v3_orig_rotate_block_state = rotate_block_state

    def rotate_block_state(state, row: int, col: int, player=None):
        if _core_folds(state):
            raise ValueError("Core 5x5 folds 2x2 rotation.")
        return _wt_core_v3_orig_rotate_block_state(state, row, col, player)
except Exception:
    pass

try:
    _wt_core_v3_orig_swap_market_tile = swap_market_tile

    def swap_market_tile(state, letter: str = ""):
        if _core_folds(state):
            raise ValueError("Core 5x5 folds Relief Swap / reading exchange.")
        return _wt_core_v3_orig_swap_market_tile(state, letter)
except Exception:
    pass

try:
    _wt_core_v3_orig_preview_move = preview_move

    def preview_move(state, row: int, col: int, letter: str, path):
        response = _wt_core_v3_orig_preview_move(state, row, col, letter, path)
        try:
...
def wt_intelligibility_core_v3_summary():
    try:
...
# WT_LIBERTY_REFINED_PREVIEW_V2_BEGIN
# Read-only pressure layer:
# - compute_group_liberties(): WT escape-route count, not strict Go liberty.
# - liberty_status(): 4-stage pressure label.
# - preview_move wrapper: adds future-facing escape route X->Y and double-move feedback.
# Rules/scoring/capture/bot choice are unchanged.

def compute_group_liberties(state, owner):
    # Connected owner groups with adjacent escape-route count.
    # WT definition:
    #   escape route = adjacent owner-less / neutral / empty cell.
    #   opponent-owned cells are not exits.
    #   fortified cells are not exits.
    # Read-only: does not affect capture rules.
    board = getattr(state, "board", []) or []
    n = len(board)
        ...
        for c in range(len(board[r])):
            if getattr(board[r][c], "owner", None) == owner and (r, c) not in visited:
                ...
                while stack:
                    ...
                    for nr, nc in neigh(cr, cc):
                        ncell = board[nr][nc]
                        o = getattr(ncell, "owner", None)
                        if o == owner and (nr, nc) not in visited:
                            stack.append((nr, nc))
                        elif o is None and not bool(getattr(ncell, "fortified", False)):
                            libs.add((nr, nc))
...
try:
    _wt_liberty_refined_orig_preview_v2 = preview_move

    def preview_move(state: GameState, row: int, col: int, letter: str, path) -> PreviewMoveResponse:
        response = _wt_liberty_refined_orig_preview_v2(state, row, col, letter, path)
        ...
        try:
            ...
            try:
                ...
                response.captureGain = cap
                response.pathGain = path_gain
                response.lockGain = int(getattr(last, "fortifiedCellsGained", 0) or 0)
                response.captureHappened = cap > 0
                response.captureCount = int(getattr(last, "captureCount", 0) or 0)
                response.comboLabels = list(getattr(last, "comboLabels", []) or [])
                # Tuned from attached measurement: avoid over-labeling tiny path+capture moves.
                response.doubleMove = bool(path_gain >= 3 and cap >= 1)
            except Exception:
                pass
            ...
            for cell, after_lib in after_map.items():
                ...
                if drop > best_drop or (
                    drop == best_drop and drop > 0 and
                    (best_after is None or int(after_lib) < int(best_after))
                ):
                    best_drop = drop
                    best_before = int(before_lib)
                    best_after = int(after_lib)

            if best_after is not None and best_drop > 0:
                try:
                    response.enemyLibertyBefore = int(best_before or 0)
                    response.enemyLibertyAfter = int(best_after or 0)
                    response.nearEncircle = int(best_after or 0) <= 1
                    response.enemyLibertyStatus = liberty_status(int(best_after or 0))
                except Exception:
                    pass

        except Exception:
            # Preview must never break normal input.
            pass
...
# WT_LIBERTY_REFINED_PREVIEW_V2_END


# WT_LIBERTY_REFINED_PREVIEW_V3_BEGIN
# Read-only pressure layer.
# Adds future-facing 逃げ道 / liberty values to preview only.
# Does not change scoring, legal moves, capture rules, or bot rules.
...
try:
    _wt_liberty_refined_orig_preview_v3 = preview_move

    def preview_move(state: GameState, row: int, col: int, letter: str, path) -> PreviewMoveResponse:
        response = _wt_liberty_refined_orig_preview_v3(state, row, col, letter, path)
...
# WT_MULTI_IMPACT_V4_BEGIN
# Multi-Impact Move System v4.
# Purpose: keep only true gameplay peaks. Score-neutral.

def _wt_mi_safe_region_count_v4(state, player: str) -> int:
    try:
...
def _wt_mi_liberty_stats_v4(state, player: str):
    try:
        groups = compute_group_liberties(state, player)
    except Exception:
        return {"groups": 0, "min": 99, "near": 0, "atari": 0}
    if not groups:
        return {"groups": 0, "min": 99, "near": 0, "atari": 0}
    libs = []
    for g in groups:
...
def _wt_mi_score_diff_v4(state, player: str) -> int:
    opp = other_player(player)
    try:
...
def _wt_multi_impact_labels_v4(before_state, after_state, player: str, word: str, delta: dict, existing_labels: list[str]) -> list[str]:
    opp = other_player(player)
    ...
    before_my_regions = _wt_mi_safe_region_count_v4(before_state, player)
    after_my_regions = _wt_mi_safe_region_count_v4(after_state, player)
    if territory_gain >= 3 and before_my_regions >= 2 and after_my_regions < before_my_regions:
        effects.append("接続")

    before_opp_regions = _wt_mi_safe_region_count_v4(before_state, opp)
    after_opp_regions = _wt_mi_safe_region_count_v4(after_state, opp)
    if territory_gain >= 3 and after_opp_regions >= before_opp_regions + 2:
        effects.append("大分断")

    b_lib = _wt_mi_liberty_stats_v4(before_state, opp)
    a_lib = _wt_mi_liberty_stats_v4(after_state, opp)
    if a_lib["atari"] >= b_lib["atari"] + 2 or (b_lib["min"] >= 4 and a_lib["min"] <= 1):
        effects.append("強いOPEN詰め")
    ...
    if board_effect_count >= 2 and ((lang == "ja" and wl >= 7) or (lang != "ja" and wl >= 8)):
        effects.append("長語")
    ...
    before_diff = _wt_mi_score_diff_v4(before_state, player)
    after_diff = _wt_mi_score_diff_v4(after_state, player)
    swing = after_diff - before_diff

    # Do not let ordinary multi-impact moves or old Bot labels become comeback drama.
    if before_diff <= -5 and after_diff >= 2 and swing >= 12 and (capture_count >= 3 or territory_gain >= 7):
        out.append("大逆転の一手")
...
# WT_SECOND_PLAYER_KOMI_6_FINAL_OVERRIDE_V1_BEGIN
# Dynamic second-player komi: +6.0.
# This is intentionally placed at the end of engine.py so these definitions win.
# Bot/evaluation calls total_score(), which remains RAW.
# Final winner calls komi_adjusted_total_score(), which gives the second player +6.0.

_SECOND_PLAYER_KOMI_6_FINAL_OVERRIDE_V1 = 6.0

def _wt_starting_player_komi6_v1(state: GameState) -> str:
    sp = getattr(state, "startingPlayer", None)
    return sp if sp in ("RED", "BLUE") else "RED"

def _wt_second_player_komi6_v1(state: GameState) -> str:
    return "BLUE" if _wt_starting_player_komi6_v1(state) == "RED" else "RED"

def _wt_second_player_komi_amount_komi6_v1(state: GameState, player: str) -> float:
    try:
        komi = float(getattr(state, "secondPlayerKomi", _SECOND_PLAYER_KOMI_6_FINAL_OVERRIDE_V1))
    except Exception:
        komi = _SECOND_PLAYER_KOMI_6_FINAL_OVERRIDE_V1
    return komi if player == _wt_second_player_komi6_v1(state) else 0.0
...
def komi_adjusted_total_score(state: GameState, player: str) -> float:
    return raw_total_score(state, player) + _wt_second_player_komi_amount_komi6_v1(state, player)
