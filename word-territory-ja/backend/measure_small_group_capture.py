# measure_small_group_capture.py - small group-capture diagnostic
...
import engine
MODE=sys.argv[1] if len(sys.argv)>1 else "quick"; GAMES=int(sys.argv[2]) if len(sys.argv)>2 else 60
def build(mode):
...
def sync(st):
    ...
    if callable(f): f(st)
def run():
    events=0; caps=[]; maxcaps=[]; enabled=bool(getattr(engine,"_WT_SMALL_GROUP_CAPTURE_ENABLED",False))
    for _ in range(GAMES):
        st=build(MODE); sync(st); mx=0
        for _t in range(60):
            try: st=engine.apply_bot_move(st)
            except Exception: break
            last=st.moveHistory[-1] if st.moveHistory else None; cap=int((last.captureCount or 0) if last else 0)
            if cap>0: events+=1; caps.append(cap); mx=max(mx,cap)
            if getattr(st,"winner",None): break
        maxcaps.append(mx)
    avg=lambda x: statistics.mean(x) if x else 0
    print("=== SMALL GROUP CAPTURE DIAGNOSTIC ===")
